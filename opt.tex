...
\gs{p^{**}(0)=\sup_{\lambda} \left( \langle 0, \lambda \rangle - u^*(\lambda) \right)=\sup_\lambda \left( -F^*(0,\lambda) \right)=-\inf_\lambda F^*(0,\lambda)}This can be viewed as the promgram associated with $F^*$. 
...
The dual program of \ref{opt} is to minimize $-p^*$, it can be written as \gs{
    \inf_\lambda (L(\lambda))\st \lambda\le_{K^*}  0\label{optt}\tag{$P^*$}
}  In the hemispace case, the polar cone and dual cone is $$H^0=[0,\infty]^p\times \bar\R^q,H^*=-H^0=[-\infty,0]^p\times \bar\R^q$$

There is a more genral duality theorem\cite{roc}: one have \gs{\inf (f-g)=\sup\yk{ g_*-f^*}}when $f,g,f-g$ are proper. \redd{TBA}
...
\gs{p^*(\lambda,\mu)=-L(\lambda,\mu)+I_{H^0}(\lambda,\mu)}
 Recall our dicussion on perturbation and strong duality in \ref{sec:perturbation}. The condition $0\in \ri\yk{\text{dom}(p)} $ is equivalent to the following condition: there is a point $x$ s.t. $g_0(x)=0,g_1(x)<0,Ax=b$ where $g_0$ is the affine part of $g$. This is the \textbf{Relaxed Slater's qualification} for the problem \ref{opt}. 
...
When Slater's condition holds, the strong duality holds, and the KKT condition is necessary and sufficient for optimality. Note that MFCQ is a special case of this.

The KKT condition for convex problems is the same as before but with the differential replaced by subdifferential:
\[\begin{aligned}
&\text{Stationarity:} &&0\in \partial_x         L(x,\lambda,\mu) =\partial f(x)+\lambda\cdot\nabla A+\mu\cdot \nabla g(x)\\  
&\text{Primal feasibility:} && g(x)\le 0,\ Ax=b\\
&\text{Dual feasibility:} && \mu\ge 0\\ 
&\text{Complementary slackness:} && \mu\cdot g(x)=0
\end{aligned}\]

We shall show that under Slater's condition, the KKT condition is sufficient and necessary for optimality. (\redd{TBA})
